...
exports.checkLoginEntry = function(UserName, password, completeWithStatus) {
   ...
   //Works Fine, but assumed different query
   /*exports.view = function(user,course,completeWithStatus)
   {
    console.log("Entering db.js");
        //connection.query('SELECT `1a`,`1b`,`1c`,`2a`,`2b`,`2c`,`3a`,`3b`,`3c`,`R1`,`R2`,`R3`,`R4`,`R5`,`R6`,`R7`,`R8`,`R9` FROM `student_details` WHERE `USN`=123 AND Course = ?', [course], (err,results,fields) =>
        connection.query('SELECT `1a`,`1b`,`1c`,`2a`,`2b`,`2c`,`3a`,`3b`,`3c`,`R1`,`R2`,`R3`,`R4`,`R5`,`R6`,`R7`,`R8`,`R9` FROM `student_details` WHERE `UserName`= ? AND `Course` = ?', [user,course] ,(err, results, fields) => 
        {
            if (err) 
            {
